...
- Step 4 の rmse 10〜12cm 台の床は、間引き量ではなく front マップ側のスミア（Step 3 の voxel サイズ・deskew 精度）に起因する可能性が高い。→ 下記「Step 3: 近傍ボクセル欠落問題の調査と修正」で一部着手・解消したが、根本の occ 中央値の低さ（ボクセル解像度とビーム密度のミスマッチ）は未解決のまま残る。
