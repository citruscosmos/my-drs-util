...
### 残留する系統誤差：カメラドライバ遅延（2026-07-09: 実証・実装）

カメラドライバが露光完了後にタイムスタンプを付与する場合、
実際の露光時刻より数 ms〜数十 ms 遅れた値が記録される。
LiDAR の `t_us` とは異なり、カメラには（本来は）per-frame の実露光時刻情報がないため、
この遅延はアーキテクチャで自動吸収できない系統誤差として残り得る、というのが当初の設計判断だった。

```
10km/h = 2.78 m/s で Δt = 10 ms の遅延 → 位置誤差 ≈ 28 mm
```

**仮説検証（fix-RTK バッグ、camera1、2026-07-09）**: 本バッグは最大 17.6km/h・13.9% の時間が
10km/h 超と速度変化が大きく、レイテンシ仮説を検証する好条件だった。速度は二面性を持つ
（単眼三角測量には視差＝並進ベースラインが必要で速度が有利、一方レイテンシ由来の位置誤差
`= 速度 × Δt` は速度に比例して悪化）。camera1 の観測ごとの局所車速と、(1) LiDAR マップ照合後の
再投影残差、(2) LiDAR マップに非依存な三角測量の内部整合性 `reproj_norm` の相関を、視差角で
層別して交絡を制御して計測したところ、**同一視差角帯でも高速の方が一貫して誤差が小さい**
（レイテンシ仮説と逆方向）という結果になった。視差改善効果がレイテンシ由来の悪化を上回って
覆い隠している可能性は残るため完全には否定できないが、**このバッグ・このカメラでは
レイテンシは支配的要因ではない**と判断。

**判明した非対称性**: LiDAR は点単位で `time_stamp` 補正済み（上記節）だが、カメラ側
（`step5_v3_cam_cloud.py` の `klt_track`）はフレーム全体で単一の `header.stamp` しか
使っておらず、フレーム内のローリングシャッターによる行毎の時刻ズレは無補正だった。

**実装**: 単眼の場合、「LiDAR とカメラの時刻差」自体は登録先が蓄積された静的マップである
限り問題にならない（上記「カメラ：蓄積マップ + RTK 補間による吸収」節の設計通り）。問題になり
得るのは `header.stamp` 自体が真の撮影時刻を正確に表しているかどうかで、ここにローリング
シャッター起因のズレが乗る。ハードウェア仕様（センサー種別ごとに実測・提供された値、
`frame_period = readout_time + fixed_exposure` で全モデル完全一致を確認済み）を用いて、
観測画素の行 v から `(v - height/2) * line_scan_s` の行補正を `header.stamp` に加えるよう
`step5_v3_cam_cloud.py` の `rolling_shutter_t_ns()` を実装（トラック保存時に一度だけ焼き込み、
以降のパイプラインは透過的に補正済み時刻を使う）。

| センサー | 対象カメラ | 1フレーム | 露光(固定) | 1行走査 | 総行数 | 読み出し時間 | 中央行からの最大オフセット |
|---|---|---|---|---|---|---|---|
| C3 | camera0,1,4,5 | 54.2ms | 11ms | 0.02ms | 2160 | 43.2ms | ±21.6ms |
| C2 | camera2,3,6,7 | 34.25ms | 11ms | 0.0125ms | 1860 | 23.25ms | ±11.6ms |
| C1（魚眼） | camera8,9,10,11 | 41.464ms | 11ms | 0.0238ms | 1280 | 30.464ms | 補正対象外（`header.stamp` が中央行基準に校正されていないため） |

併せて、三角測量に使う観測を `v3_triangulate_obs_stride`（既定 4）で間引き、隣接フレーム間の
近すぎるベースラインによる三角測量ノイズを抑制（最終的な reprojection 精密化は全観測を使用、
間引くのは三角測量のレイ交差計算のみ）。

**検証結果（camera1）**: 補正・間引きとも実装として正しく機能していることを確認（例: v=0 で
-21.6ms、v=2159 で +21.58ms、中央行 v=1080 で 0ms の理論通りの補正量）。しかし実測・self-test
双方で残差・snap率・摂動回復性に**有意な変化は観測されなかった**（詳細は「v3 fix-RTK 実データ
検証」節）。副作用もないため実装は維持するが、camera1 に関する限りレイテンシは残差の支配的
要因ではないという上記の結論と整合する。

当初想定していた `camera_timestamp_offset_sec`（カメラ毎の 1D 探索で推定するフラットな定数
オフセット、v3-M3 として設計）は、この行毎のローリングシャッター補正で置き換えられる形となり
**実装しない**（後述チューニングパラメータ一覧を参照）。
...
- **検証（2026-07-03 実施・本設計の根拠）**（bag: `HRdqo3pf_2026-06-10T15-22-51.mcap`, 全区間 stride10 → front マップ）:
  ...
  8. **旧来のキャリブレーション手法で求めた TF の精度検証（2026-07-05 実施）**: `step4_lidar_to_lidar.py` に **`--eval-only` モード**（最適化を一切行わず、与えた tf を本パイプラインと同一の項A＋項B コスト・`overlay_gap_stats` で評価するだけのモード）を追加し、以前の手法で求めた TF 値（`tools/auto-calib/prev_calib_tf.yaml`。front は受け入れ済み校正値 x=0.299/y=0.0/z=0.037 と一致）を同一 bag（`HRdqo3pf_2026-06-10T15-22-51.mcap`）・同一 front マップ上で評価した（`docs/previous-method-result/`）。結果:

     | lidar | rmse (点面) | overlay gap p50/p90/p99/max [cm] |
     |---|---|---|
     | right | 11.51cm | 4.7 / 43.4 / 170.6 / 308.5 |
     | left | 13.05cm | 5.1 / 32.8 / 136.7 / 305.3 |
     | rear | 12.62cm | 4.1 / 30.1 / 106.4 / 636.8 |

     本パイプラインの最適化結果（GT 照合: right 2.7cm・rear 9.1cm）と比べ、旧手法はいずれも rmse で数倍〜一桁大きい。中央値（p50 4〜5cm）はマップ自体のスミア下限に近い一方、p90/p99/max が数十〜600cm超まで裾を引いており、視覚的にも無視できない誤配置がある（`step4_eval_{right,left,rear}_overlay_topdown.png` / `step4_eval_combined_overlay_topdown.png`）。旧手法の絶対精度は本パイプラインの結合最適化（項A＋項B）に劣ることを定量的に確認した。
...
#### v3 fix-RTK 実データ検証（2026-07-09、camera1）

**使用バッグ**: `HRdqo3pf_2026-06-25T17-37-26_pc_replaced_rtkint.mcap`（136s、駐車場）。Step 0 検証
PASS（fatal/warning 0件）、**RTK covariance 中央値 0.000256 m²**（≈1.6cm std）— 従来検証した図8
バッグの float 解（~0.2 m²）から2桁近く改善しており、fix 解相当のデータと判断。速度 median
1.86 m/s・max 4.89 m/s（17.6km/h）、yaw 励起 583°。Step 0-4 をこのバッグに通し、Step 4 RMSE は
right 7.25cm / left 8.97cm / rear 10.51cm（既存の「~10cm 環境フロア」と整合）で正常終了。

**M0（識別可能性チェック）が float 解から改善**: camera0 5/6 GO・camera4 4/6 GO
（従来の float 解 8の字では camera4 が 0/6 で不合格だった）。float RTK ノイズが rear 側の
識別可能性を阻害していたという仮説を裏付ける結果。

**Step 5 v2（camera0/4）で発見・修正したメモリリーク**: 全区間フレーム（544枚/カメラ、
3840×2160 の生解像度）の Distance Transform を全て同時保持する設計だったため、coarse-to-fine
の Gaussian blur パスで一時的に二重コピーが生じ、1ワーカーあたり 25GB・50GB まで RSS が膨張して
OOM Kill → multiprocessing.Pool のデッドロックでハング、という不具合を発見。`image_scale`
（新規実装、既定 0.5）で最適化前に画像を縮小し、RSS ピークを 2 ワーカー合計 27.4GB まで低減して
解消（4Kネイティブ解像度のまま保持する設計自体が破綻していたため、縮小は速度目的の任意設定
ではなく必須の修正）。修正後、camera0 resid 7.39→7.25px（HIGH-RESIDUAL）・camera4 resid
2.10→2.04px（HIGH-RESIDUAL）で完走を確認（他ステップ・他ファイルに同様の全フレーム同時保持
パターンがないかも確認済み：v3 の KLT 追跡は真のストリーミング実装、M0 は8ペアのみ保持で
いずれも該当なし）。

**Step 5 v3（camera1）self-test で摂動回復失敗を確認・原因を定量特定**: 未摂動（x0=tf_static
起点）では残差 38.4→14.3px・snap率 63.7%・補正量 8.2cm/0.24° で滑らかに収束する一方、
`--self-test`（±10cm/3° の既知摂動）では残差が 25px 付近で完全に横ばい、snap率が 63%→39% に
崩壊し、摂動開始点からほとんど動かなかった。

原因を camera1 の三角測量点の深度分布（中央値 19.4m、p90 48.6m、p99 100.5m）から定量検証:
回転誤差は `depth × tan(θ)` で横方向位置誤差に変換されるため、**わずか 3° の誤差で全点の
84.8% が固定スナップ閾値 0.5m（`v3_snap_max_dist`）を超える**。遠くまで見える広FOVカメラでは、
数度の外パラ誤差だけで reconstruct→snap→refine の外側ループが対応点（＝修正力）の大半を
失い、収束基底が非常に狭いことが判明（実装バグではなく、固定閾値の point-to-plane 対応付けに
構造的に内在する ICP 的限界）。

**修正1: snap 距離の coarse-to-fine 化**（`v3_snap_max_dist` → `v3_snap_max_dist_schedule`
`[3.0, 1.5, 0.75, 0.5]`、`v3_outer_iters` を 3→5 に増加）。効果:

| ケース | 残差RMS(px) | snap率 | x0からの距離 |
|---|---|---|---|
| 実測（修正前・固定0.5m） | 38.4→14.3 | 63.4% | 5.7cm/0.15° |
| 実測（修正後・coarse-to-fine） | 38.4→14.3 | 63.7% | 8.2cm/0.24° |
| 摂動10cm/3°（修正前） | 26.3→25.4（横ばい） | 38.9% | 2.7cm/2.91°（回復せず） |
| 摂動10cm/3°（修正後） | 60.8→**23.4**（単調減少） | 44.4% | 21.7cm/2.68°（部分改善だが未回復） |
| 摂動3cm/1°（修正後） | 41.2→**15.6** | 61.1% | 6.4cm/0.79°（実測に近い値まで回復） |

残差が単調に減少するようになり、現実的な規模の誤差（3cm/1°程度）からは正しく回復するように
なったが、self-test既定の 10cm/3° という大きな摂動からは依然として完全回復しない（誤った
local optimum に収束）。**実運用上の外パラ誤差は 10cm/3° よりずっと小さいと想定されるため、
「小さな残差誤差の精密化」用途としては修正後の設計で機能する。**

**修正2: ローリングシャッター行補正 + 三角測量観測の間引き**（詳細は「残留する系統誤差：
カメラドライバ遅延」節）。実装は正しく機能していることを確認したが、実測・3cm/1°摂動・
10cm/3°摂動のいずれのケースでも修正1からの**有意な変化は観測されなかった**（例: 実測で
残差 38.4→14.3px→36.0→14.5px、10cm/3°摂動で 21.7cm/2.68°→21.0cm/2.72°）。camera1 に関する
限り、レイテンシ・三角測量ベースラインは残差の支配的要因ではないと判断。

**点群の目視検証**: LiDAR マップ（6万点にダウンサンプル）と camera1 の三角測量点群
（修正前後）を WebGL の3Dビューアで視点移動しながら比較。**同一の静止構造物を捉えられている
ことを確認**し、カメラ点群の生成（マップ化）自体は健全であるという結論を裏付けた。

**総合結論**: v3 単眼カメラ登録は「tf_static が既に妥当な範囲にある前提での小さな誤差の
精密化」としては機能する（camera1 で残差 14.3px・snap率 64% まで安定収束）。一方
「大きくズレた初期値を単眼カメラ1台だけで力ずくで正す」用途には、reconstruct→snap→refine の
ICP的性質上、構造的な限界がある（coarse-to-fine化で収束基底は拡張したが、根本解消はしていない）。
これは Step 6 のカメラ間クロスビュー残差（狭FOVで既に機能している camera0 等の高信頼構造を
広FOVカメラの補強材料に使う設計、後述）の動機を補強する結果である。
...
**2026-07-09 追記（動機の補強）**: v3 の fix-RTK 実データ検証（camera1、詳細は「v3 fix-RTK 実データ
検証」節）で、単眼カメラ1台の reconstruct→snap→refine は「小さな誤差の精密化」はできるが
「大きな初期誤差からの回復」には構造的に限界がある（coarse-to-fine 化で改善したが未解消）ことが
定量的に確認された。camera1（120°、front クラスタ）は v2 で既に良好に機能している camera0（30°）
と同クラスタであり、まさにこの設計が想定する「狭FOVカメラの高信頼構造で広FOVカメラの弱い識別性を
補う」状況に合致する。Step 6 着手時の優先候補として記録。
...
| **Step 5a/5b** | | | | |
| `camera_timestamp_offset_sec` | ⬜未実装 | — | — | 設計時に想定していたカメラ毎のフラットな遅延オフセット（1D探索で推定、v3-M3）。2026-07-09: v3 についてはローリングシャッター行補正（`step5_v3_cam_cloud.py` の `rolling_shutter_t_ns`、ハードウェア仕様ベースの固定値、後述）で代替・実装したため本パラメータは実装しない |
| `cam_frame_stride` | `5` | `2` | `10` | 最適化フレーム間引き率 |
| `cam_parallel_workers` | `4` | `4` | `8` | カメラ並列処理数 |
| `image_scale` | `0.5` | `0.5` | `0.35` | 最適化時の画像縮小率。2026-07-09: 全フレーム分の Distance Transform をネイティブ解像度（4K）のまま同時保持すると 1 ワーカーあたり RSS が数十GBに達し OOM する実バグを確認したため、デフォルトを 1.0→0.5 に変更（任意の速度最適化ではなく必須の修正） |
...
| **Step 5 v3**（他パラメータは「v3 先行実装・事前検証まとめ」節参照） | | | | |
| `v3_snap_max_dist_schedule` | `[3.0, 1.5, 0.75, 0.5]` | `[2.0, 1.0, 0.5, 0.5]` | `[3.0, 0.5]` | LiDARマップへのスナップ距離 [m] を outer iter 毎に coarse→fine（2026-07-09: 固定 0.5m は数度の回転誤差で大半の点が対応不能になり摂動回復が破綻する不具合があり、スケジュール化で収束基底を拡張。範囲を超える反復では最終値を維持） |
| `v3_outer_iters` | `5` | `8` | `3` | reconstruct↔register 外側反復回数（2026-07-09: 3→5。coarse-to-fine スケジュールを最後まで通すのに必要） |
| `v3_triangulate_obs_stride` | `4` | `2` | `8` | 三角測量のレイ交差計算に使う track 内観測の間引き率（2026-07-09 追加）。隣接フレームは近すぎてベースラインが小さくノイズに弱いため。最終 reprojection 精密化は全観測を使用 |
| **Step 6** | | | | |
...
├── step5_cam_to_lidar.py        # camera0-11 共通（通常+魚眼）✅ v2(scan-to-image遮蔽エッジ)実装済み・fix解でcamera0/4完走確認（`image_scale`でOOM修正、後述）
├── step5_m0_check.py            # Step5 v2 M0: 識別可能性スイープ（scan-to-image、遮蔽/平面エッジ+MI）✅ 実装・検証済み・fix解でcamera4もGOに改善
├── step5_v3_cam_cloud.py        # Step5 v3 part1: KLT追跡+多視点三角測量（camera cloud構築）✅ 実装・fix解camera1で検証済み（ローリングシャッター行補正・三角測量間引き追加）
├── step5_v3_register.py         # Step5 v3 part2: LiDARマップへ登録（reprojection GN, Step4流用）✅ 実装・fix解camera1で検証済み（snap距離coarse-to-fine化で摂動回復基底を拡張）
...
| 5 Camera-LiDAR | 🔶 v2/v3 併存・固定ルーティング決定済み・**fix解でcamera0/4(v2)+camera1(v3)を実データ検証済み（2026-07-09）** | v1（DT最小化）棄却。v2 M0 で **float 解8の字では front のみ識別可（3/6）、他はシーン内容で NO-GO** と実証。→ v3（RTK fix 前提・全軌道蓄積カメラ点群→LiDAR登録）を実装、5カメラ横断検証で **広FOV・魚眼ほど幾何学的条件が改善**（M0とは逆傾向＝相補的）も**float解では摂動回復は全カメラ未達**。**2026-07-04: camera0,4→v2／それ以外→v3 の固定ルーティングを決定**。**2026-07-09: fix解バッグでM0再検証（camera4も4/6 GOに改善）、v2でOOMバグ発見・修正（`image_scale`）、v3(camera1)でsnap距離coarse-to-fine化により摂動回復基底を拡張（3cm/1°は回復・10cm/3°は未回復）、レイテンシ仮説を検証（ローリングシャッター行補正を実装するも有意な改善なし＝支配要因ではないと判明）**。詳細は Step 5 v3 節「fix-RTK 実データ検証」参照 |
